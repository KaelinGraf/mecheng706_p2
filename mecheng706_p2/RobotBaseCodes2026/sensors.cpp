...
float Ultrasonic::readBlocking() {
  ...
  t1 = micros();
  t2 = t1;   // seed so a zero-length echo can't underflow pulse_width = t2 - t1 (unsigned)
  ...
  // Calculate distance in centimeters. The constant is from the datasheet,
  // calculated from the assumed speed of sound in air at sea level (~340 m/s).
...
float Gyroscope::readSensor(){
  ...
  if (_bno08x->getSensorEvent(_sensorValue) &&
    ...
    // Yaw (about Z) from the already-fused quaternion. The BNO MotionEngine filters this on-chip, so
    // no extra smoothing (and averaging a WRAPPED angle would be wrong across +/-pi -- the old branch's
    // RingBuffer average did exactly that). Unwrap the per-sample delta into a CONTINUOUS heading.
